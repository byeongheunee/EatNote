...
import com.ssafy.eatnote.model.dto.ApiResponse;
...
public class AuthController {
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequest loginRequest) throws Exception {
        ...
        if (user == null || !user.getPassword().equals(password)) {
        	return ResponseEntity.status(401).body(ApiResponse.failure("INVALID_CREDENTIALS", "이메일 또는 비밀번호가 틀렸습니다."));
        }
        ...
        // 토큰 응답
        return ResponseEntity.ok(ApiResponse.success(token, "USER_LOGIN_SUCCESS", "로그인에 성공했습니다."));
    }
}
