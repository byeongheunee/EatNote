<template>
  <header
    class="bg-gradient-to-r from-yellow-50 via-orange-50 to-amber-50 backdrop-blur-md border-b border-yellow-200/50 shadow-lg sticky top-0 z-50">
    <div class="container mx-auto px-4 py-3">
      <div class="flex justify-between items-center max-w-6xl mx-auto">
        ...
        <div class="flex items-center cursor-pointer group" @click="goHome">
          <span
            class="text-2xl font-bold bg-gradient-to-r from-orange-600 to-amber-600 bg-clip-text text-transparent group-hover:from-orange-700 group-hover:to-amber-700 transition-all duration-300">
            EatNote
          </span>
        </div>
        ...
        <nav class="hidden md:flex items-center justify-center flex-1 mx-8">
          <div
            class="flex items-center space-x-1 bg-white/60 backdrop-blur-sm rounded-full px-6 py-2 border border-yellow-200/50">
            <!-- 로그인 안한 경우 -->
            <template v-if="!isLogin">
              <RouterLink :to="`/community/${defaultBoardId}`"
              ...
              <button @click="$emit('go-feature')"
                class="flex items-center gap-2 px-4 py-2 text-gray-700 hover:text-orange-600 hover:bg-white/80 rounded-lg transition-all duration-300 font-medium">
                <span>✨</span> 주요기능
              </button>
              <button @click="$emit('go-usage')"
                class="flex items-center gap-2 px-4 py-2 text-gray-700 hover:text-orange-600 hover:bg-white/80 rounded-lg transition-all duration-300 font-medium">
                <span>📖</span> 이용방법
              </button>
            </template>
            ...
            <template v-else>
              ...
              <template v-else-if="user.userType === 1">
                <RouterLink to="/trainer/feedback"
                  class="flex items-center gap-2 px-4 py-2 text-gray-700 hover:text-orange-600 hover:bg-white/80 rounded-lg transition-all duration-300 font-medium">
                  <span>💪</span> 나의 피드백
                </RouterLink>
              </template>
        ...
        <div class="flex items-center space-x-3">
          ...
          <template v-if="isLogin">
            <div
              class="hidden sm:flex items-center space-x-2 bg-white/60 backdrop-blur-sm rounded-full px-3 py-1.5 border border-yellow-200/50">
      ...
      <div v-if="showMobileMenu"
        class="md:hidden mt-4 p-4 bg-white/80 backdrop-blur-sm rounded-2xl border border-yellow-200/50 shadow-lg">
        <div class="space-y-3">
          <!-- 로그인 안한 경우 -->
          <template v-if="!isLogin">
            <RouterLink :to="`/community/${defaultBoardId}`" @click="showMobileMenu = false"
            ...
            <button @click="$emit('go-feature'); showMobileMenu = false"
            ...
            <button @click="$emit('go-usage'); showMobileMenu = false"
              class="flex items-center gap-3 p-3 text-gray-700 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition-all duration-300 font-medium w-full text-left">
          ...
          <template v-else>
            <!-- 사용자 정보 -->
            <div
              class="flex items-center space-x-3 p-3 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-xl border border-yellow-200/50">
              <img :src="getProfileImage(user.profileImage)" alt="프로필"
            ...
            <!-- 일반회원 메뉴 -->
            <template v-if="user.userType === 2">
              <RouterLink to="/meals" @click="showMobileMenu = false"
                class="flex items-center gap-3 p-3 text-gray-700 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition-all duration-300 font-medium">
                <span class="text-lg">🍽️</span> 식단
              </RouterLink>
              <RouterLink to="/videos" @click="showMobileMenu = false"
                class="flex items-center gap-3 p-3 text-gray-700 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition-all duration-300 font-medium">
            ...
            <!-- 트레이너 메뉴 -->
            <template v-else-if="user.userType === 1">
              <RouterLink to="/trainer/feedback" @click="showMobileMenu = false"
                class="flex items-center gap-3 p-3 text-gray-700 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition-all duration-300 font-medium">
