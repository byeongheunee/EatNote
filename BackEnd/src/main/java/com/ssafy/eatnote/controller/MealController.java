...
public class MealController {
	...
	@ApiResponses({ @ApiResponse(responseCode = "200", description = "정상적으로 식단 상세 정보를 반환했습니다."),
			@ApiResponse(responseCode = "404", description = "해당 식단을 찾을 수 없습니다.") })

	@GetMapping("/{mealId}")
	public MyApiResponse<MealPublicDetailResponse> getPublicMealDetail(@PathVariable Long mealId,
	...
	public MyApiResponse<?> uploadMealImage(@RequestParam("file") MultipartFile file,
			@RequestParam(value = "mealTime", required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime mealTime,
			@AuthenticationPrincipal UserDetailsImpl userDetails) {
		...
		try {
			...
			Meal savedMeal = mealService.saveAnalyzedMeal(file, userId, mealTime);
	...
	public MyApiResponse<?> deleteMeal(@PathVariable Long mealId,
			@AuthenticationPrincipal UserDetailsImpl userDetails) {
		try {
			Long userId = userDetails.getUser().getUserId();
	...
	@ApiResponses({ @ApiResponse(responseCode = "200", description = "최근 식단 조회 성공") })
	...
	@ApiResponses({ @ApiResponse(responseCode = "200", description = "인기 식단 조회 성공") })
	...
	@ApiResponses({ @ApiResponse(responseCode = "200", description = "식단 통계 조회 성공"),
	...
	public MyApiResponse<List<MealListViewResponse>> getMyRecentMeals(
			@AuthenticationPrincipal UserDetailsImpl userDetails) {
		Long userId = userDetails.getUserId();
	...
	public MyApiResponse<List<MealFeedbackListViewResponse>> getMyRecentFeedbacks(
			@AuthenticationPrincipal UserDetailsImpl userDetails) {
		Long userId = userDetails.getUserId();
